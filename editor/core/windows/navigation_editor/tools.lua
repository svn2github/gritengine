...
hud_class `Tools` (extends(WindowClass)
{
	...
	show_temp_obstacle_tool = function(self)
		...
		self.tool = create_gui_object({ parent = self, size = vec(100, 100), alpha = 0, colour = vec(1, 0, 1), align = vec(0, 1), expand_x = true })
	...
	show_offmesh_tool = function(self)
		...
		self.tool.radiobuttons[0] = create_radiobutton({caption = "One way", parent = self.tool, onSelect = function(self)  end, align = vec(-1, 0), offset =  vec(10, 0)})
		self.tool.bxsz:addChild(self.tool.radiobuttons[0])
		
		self.tool.radiobuttons[1] = create_radiobutton({caption = "Bidirecional", parent = self.tool, onSelect = function(self)  end, align = vec(-1, 0), offset =  vec(10, 0)})
