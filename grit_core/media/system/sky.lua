...
--    ]];
    ...
    fragmentCode = [[
        val colour = pma_decode(sample2D(mat.emissiveMap, vert.coord0.xy))
                     * Float4(1, 1, 1, mat.alphaMask);
        if (colour.a <= mat.alphaRejectThreshold) discard;
        frag.colour = Float4(gamma_decode(colour.rgb) * mat.emissiveMask, colour.a);
    ]];
}
...
--    ]];
    ...
    fragmentCode = [[
        ...
        // STARFIELD
        frag.colour.rgb = mat.starfieldMask * gamma_decode(sample2D(mat.starfieldMap, uv).rgb);
